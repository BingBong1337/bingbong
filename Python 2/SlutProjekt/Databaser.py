#Importar alla bibliotek som jag kommer använda mig av
...
#skapar ett grafiskt gränsnitt och ansluter till min databas
root = tk.Tk()
...
#Gör en klass för de elements som ska ingå i en bokning
class Booking():
...
#Bestämmer resans avgångstid och angiver den i (hh-mm)
...
#Bestämmer resans avgångsdatum och anger den i (YYYYMMDD)
...
#Bestämmer en resans avgångsstation
def TravelFromCity():
    global FromCity
...
#bestämmer en resans destination
def TravelToCity():
    TravelCityList = ['Stockholm','Göteborg','Linköping','Malmö','Uppsala','Örebro','Helsingborg','Jönköping','Norrköping','Lund','Umeå','Gävle','Södertälje','Karlstad','Östersund','Mora','Bordlänge','Solna','Halmstad','Nybro','Bålsta','Västerås']
...
#Bestämmer resans pris beroende på klass och avgångstid, kvälls/nattåg blir billigare
def TravelPrice():
    Price = 0
    if Klass == 'Economy':
        Price += 300
...
#Genererar alla bokningar i form av klassen definierad ovan
AllBookingsList = []
for i in range(500000): #Sätt till 500000 innan testning!
...
#Gör en sammling funktioner som tillåter användaren att boka en resa när man och UI för att kunna göra det
def BookTravel():
    ...
    #Frågar vilken plats du befinner dig på, vilken destination och vilket datum du vill resa
    ...
    #Tar alla element som visas i en tabell och som man tryckt på
    def BookTravel():
        global DesiredTravel
        selected = AvailableBookings.focus()
        DesiredTravel = AvailableBookings.item(selected,'values')

    #Gör en event funktion till när man trycker på ett element i min tabell nedan
    def BookClicker(e):
        BookTravel()

    #Funktion som bokar resan och lägger till den i databasen
    def ActuallyBook():
        ...
        BookTripBtn.place_forget()
        root.after(2000,MainCanvas)

    #Lägger till alla bokningar som matchar informationen du la in i rutorna ovan
    def GetCurrentLocationAndTravelLocationAndTimeAndDate():
        ...
        #Kollar vad man skrev in för information i text rutorna och visar resor följaktligen
    ...
    #Knappar som kör funktionerna ovan samt en avbryt knapp som återställer fönstret
...
#Visar alla registrerade användare i Admin profilen i form av en tabell
def Reader():
    #Hämtar användarinfromation från databasen
    ...
    #Gör en tabell som lägger till användarinformation hämtad ovan in i tabellerna
    ...
    #Gör knappar och text rutor som återvänder till huvudsidan och som tillåter Admin att ta bort Användare eller ändra deras infromation
    ...
    #Bindar en funktion nedan till m1 mus släpp
    tree2.bind("<ButtonRelease-1>",clicker)
    return

#Funktion som uppdaterar information i databasen som Admin kan ändra
def AdminChangerGet():
...
#Lägger till det man trycker på i tabellen ovan till textruto som tillåter Admin att ändra information
def AdminChanger():
    global value
    try:
...
#Gör ett event som kör funktionen ovan när man trycker på ett element i tabellen
def clicker(e):
    AdminChanger()

#Hämtar det element man trycker på i tabellen och tillåter Admin att ta bort användaren helt
def GetRev():
...
#Gör en profil för användaren där den kan se sin infromation och kunna ändra den samt sina bokade resor som den kan avboka
def Profile():
    Canvas.delete('all')
    ...
    #Hämtar användarens information från databasen enligt vad den loggade in med
    query = ("SELECT id FROM signininfo WHERE Username = %s and Password = %s")
    mycursor.execute(query,(SignInUserName, SignInPassWord,))
    info = mycursor.fetchone()
    idresult = info[0]
    ...
    #Visar användarens infromation i textrutor som tillåter den att ändra den
    ...
    #Hämtar bokningen som man tryckt på i tabellen nedan
    ...
    #Tar bort bokningen efter vad man tryckt på i sin boknings tabell
    def DeleteBooking():
        try:
    ...
    #Skapar en tabell som visar användarens alla bokningar
    ...
    #Bindar m1 mus släpp till event funktionen ovan
    YourBookings.bind("<ButtonRelease-1>",RemoveBookingClicker)

    #Uppdaterar databasen efter om användaren ändrat sin information
    def GetNewChanges():
        name = namebox.get()
    ...
    #Skapar knappar som kör funktionerna ovan
...
#Hämtar informationen som skrivs in i loginscreenen och avgär ifall den informationen passar existerande information i databasen
def GetSignInInfo():
    ...
    try:
        #Om man loggar in som Admin kommar man till en speciell sida med speciella funktioner
        if SignInUserName == 'Admin' and SignInPassWord == 'Admin':
            AdminPage()
            AdminPrivledges += 1
        #Om inte Admin kommer man till sin egna profil
        ...
        #skriver en text om informationen inte passar existerande information i databasen
...
#Hämtar information från signupscreenen där en ny användare kan registrera sitt konto
def GetSignUpInfo():
    ...
    #Insertar angiven information i databaserna
    ...
    if AdminPrivledges >= 1:
        #Om man registrerar användare genom Admin sidan ska man återvända till Admin sidan
        AdminPage()
    else:
        #Annars återvänds man till loginscreen
        LoginScreen()

#Gör ett fönster för att kunna registrera sig
def SignUp():
    ...
    #Kollar om man försökte registrera användare som Admin men vill återvända så ska Admin komma tillbaka till Admin sidan och andra ska till Loginscreen igen
    if AdminPrivledges >= 1:
...
#Gör en loginscreen som har text rutor för att kunna logga in på sitt konto
def LoginScreen():
...
#Gör en Admin sida med speciella funktioner
def AdminPage():
    global Adder, Read

    #Hämtar information som fås genom att trycka på ett element i en tabell som visar alla resor
    def ChooseToDeleteBooking():
        global TripToBeRemoved
        try:
    ...
    #Gör en event funktion för att kunna trycka i tabellen
    def RemoveBookingClicker(e):
        ChooseToDeleteBooking()

    #Tar bort bokningar som Admin tryckt på om bokning existerar
    def DeleteBooking():
        try:
            query = ("DELETE FROM travel WHERE id = %s AND Time = %s AND Date = %s AND FromCity = %s AND ToCity = %s")
    ...
    #Gör en tabell som visar alla nuvarande bokningar
    def AllBookingsViewer():
        ...
        #Hämtar alla bokningar från databasen och lägger till den i tabellen
        ...
        #Bindar m1 mus släpp till att köra funktionen ovan
        AllBookingstree.bind("<ButtonRelease-1>",RemoveBookingClicker)

        #Gör knappar som tillåter Admin att ta bort vald bokning eller gå tillbaka till Main sidan
    ...
    LogOutBtn.place(anchor = tk.CENTER, x = 80, y = 30)
    Killer = tk.Button(root,text = 'Quit?', command = root.destroy)
    Killer.place(anchor = tk.CENTER,x=30,y=30)
    ...
    ViewChat.place(anchor = tk.CENTER, x = 1470, y = 770)
    
#Funktion som ansluter till min server
...
#Gör en funktion som när man trycker på en knapp ska rensa text rutan och skicka iväg texten till servern som skickar det till alla användare
def click_handler():
...
#Gör chatting fönstret för alla användare
def ChattingFrame():
...
#Gör en funktion som ska lämna chatten när man trycker på en knapp och ifall man är Admin återvänds man till Admin sidan och annars till sitt konto
def quitchat():
    c = 'xqzwy'
    d = c.encode("utf-16") 
    s.send(d)
...
#Gör en funktion som konstant lyssnar efter skickade medelanden
def receiver_thread():
    while True:
        try:
            b = s.recv(1024)
...
#Gör funktionen som ansluter till servern när man trycker på chatknappen 
'''
def chat():
    global s
    s = connect_to_server()
...
#Gör huvudförnstret för användaren med knappar som kör de flesta funktioner definierade i detta program
def MainCanvas():
    global BookingBtn
    Canvas.delete('all')
    ...
    Killer = tk.Button(root,text = 'Quit?', command = root.destroy)
    Killer.place(anchor = tk.CENTER,x=30,y=20)
    LogOutBtn = tk.Button(root, text = 'Logout?', command = logout)
    LogOutBtn.place(anchor = tk.CENTER, x = 80, y = 20)
    ...
    ChatBtn.place(anchor = tk.CENTER, x = 1470, y = 780)

#Börjar programet med att starta loginscreen 
LoginScreen()
root.mainloop()